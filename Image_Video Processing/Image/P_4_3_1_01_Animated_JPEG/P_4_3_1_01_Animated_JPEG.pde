...
void setup() {
  ...
  arch = loadImage("arch.jpg");//Ex_14
  println(arch.width+" x "+arch.height);
  arch.loadPixels();//Ex_14
}
...
void draw() {
  ...
  for (int gridX = 0; gridX < img.width; gridX++) {
    for (int gridY = 0; gridY < img.height; gridY++) {
      ...
      case 1:
        ...
        color z = arch.pixels[index]; // Get a pixel//Ex_14
        float r = red(z) / 3.0; // Get the red value//Ex_14
        
        float w1 = map(greyscale, 0,255, 15,0.1);
        stroke(0);
        strokeWeight(w1 * mouseXFactor);
        line(posX, posY, posX+z, posY+5);
        
        index++;//Ex_14
        if (index == 100*100) {//Ex_14
    index = 0; // Return to the first pixel//Ex_14
  }//Ex_14
  ...
  //saveFrame("output/DotFaceJPEG_####.png");
}
