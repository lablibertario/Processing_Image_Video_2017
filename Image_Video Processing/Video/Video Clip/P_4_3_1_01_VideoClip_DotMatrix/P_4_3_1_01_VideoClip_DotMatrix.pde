...
void setup() {
  size(1000, 720, P3D); //size should be multiple of img width and height. P2D causes glitches.
  smooth();
  video = new Movie(this, "Jack.mov");
  ...
  for (int gridX = 0; gridX < video.width; gridX++) {
    for (int gridY = 0; gridY < video.height; gridY++) {
      ...
      case 5:
        ...
        color c2 = video.get(min(gridX+1,video.width-1), gridY);
        stroke(c2);
